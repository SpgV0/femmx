...
metadata:
  ...
  modified: 2026-07-24T19:07:58.144Z
---
...
**Follow-up same day: "how is it done in the old gui, it looks better" +
"add the arrows in the field lines" (commit `62b26c5`).** Classic's own
vector plot (femm/FemmviewView.cpp's "Draw vectors" block) scales each
arrow's length by that element's magnitude relative to the document's
GLOBAL max (`B_High`), not a fixed unit length like femmqt's first fix
used. Matching that literally (global `m_quantityData[BMag].vMax`)
backfired on the transformer model -- one small hotspot near the coils
dominates the global max, so nearly every other arrow collapsed to an
illegible dot. Fixed by normalizing against the LOCAL max instead (only
among the current grid-sampled elements) plus a minimum-length floor
(`0.4 + 0.6*relative`) so weak-field arrows stay readable. **Same
"global reference gets dominated by one outlier region" trap as the
grayscale contrast issue and the original density-range design** --
worth checking for on sight anywhere a real (non-toy) model's field
spans a large dynamic range.

Also added small direction arrows along Contour Plot's own field lines.
Non-obvious physics shortcut worth remembering: in 2D magnetostatics the
equipotential contours of A (the field lines Contour Plot already draws)
ARE the B field lines -- B = curl(A z-hat) is everywhere tangent to a
contour of A -- so each marching-triangle crossing segment's own
containing element's B direction is directly the correct arrow direction
to draw there. No need to stitch the scattered per-triangle segments
into connected polylines first to know which way is "forward" along a
line.

**Immediate follow-up, same conversation (commit `f821325`): scope
narrowed twice in quick succession.** "add the arrows in the field
lines" (implemented always-on) -> "I only want the arrows to show in the
field lines as an option" (added `MeshSolutionItem::setShowFieldArrows`,
off by default, `View > Show Field Arrows`) -> "I do not want the vector
plot at all, remove it" (the separate `PlotMode::Vector` mode from
earlier in this same session was deleted outright: `paintVector()`, its
menu item, toolbar button/icon incl. `toolbar_icons/vector_plot.svg`
removed from disk and `icons.qrc`, and the `paint()` switch case). Net
result: the standalone Vector Plot mode this session had built and
iterated on (spatial-grid sampling fix, then local-max magnitude
scaling) is entirely gone -- field direction is shown ONLY via Contour
Plot's optional arrows now. Lesson: when a user is iterating live on a
just-added feature, each message can be a further scope cut on top of
the last one, not a new independent request -- re-read the full recent
thread before assuming the previous message's implementation is still
wanted at all.

**Same day: adopted a user-supplied color palette for both themes
(commit `8d14113`).** User attached two reference mockup images ("FEMMX
GUI -- Light/Dark Theme, 5 Color Palette") and asked to use them,
"less striking, especially when drawing and plotting". The palette is
the VS Code Dark+ scheme verbatim: Background #1E1E1E dark/#FFFFFF
light, Surface/Panels #2D2D30/#F3F4F6, Text/Primary #D4D4D4/#1E1E1E,
Primary Accent #007ACC, Secondary Accent #4EC9B0 (same hex in both
themes). Key judgment call, worth remembering if this comes up again:
BOTH reference mockups' own "Flux Density" plot legend still shows a
vivid rainbow spectrum -- read as "this 5-color palette governs the app
chrome and geometry/overlay entity colors, NOT the Density Plot's own
quantitative band color table" (`SolutionView.cpp`'s `kColorMap`/
`kGreyMap`, deliberately left untouched). `AppTheme::background()`
already matched the reference's Background color exactly in both themes
before this change (pure luck/prior good judgment, not intentional
palette-matching at the time). Rebuilt the per-entity drawing colors
(node/segment/arc/boundary-edge/mesh-line/mesh-point/hole/block-label/
selected -- previously a much wider, independently-per-theme-tuned hue
spread: pure blue, bright green, bright orange, bright red) around just
the 2 given accents plus one muted warm accent in the same VS Code
family (#CE9178, the "string" syntax color) for boundary edges, using
the SAME values in both themes now instead of separately-tuned per-theme
hues -- one consistent accent regardless of theme is itself part of
"less striking" (fewer independently-tuned hues total). Also found and
fixed one hardcoded `QColor(200,0,0)` (the block-label cross marker in
`GeometryScene.cpp`) that a plain grep for `AppTheme::` calls would have
missed entirely, since it never went through AppTheme at all -- worth
grepping raw `QColor(` construction across the whole `femmqt/` tree, not
just auditing AppTheme.cpp's own functions, whenever a "make the palette
consistent" pass like this comes up again.
