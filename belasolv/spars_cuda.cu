...
// new file. Optional CUDA-accelerated linear solve for the electrostatics
// solver, ported from fkn/spars_cuda.cu's CudaPCGSolve (the real-valued
// case only -- electrostatics has no complex/AC formulation, so
// CudaPBCGSolve doesn't apply here). Mirrors CBigLinProb::PCGSolve's
// algorithm (spars.cpp) step for step, but with a GPU-resident Jacobi
// preconditioner in place of SSOR. SSOR's forward/back substitution is a
// sequential row-by-row dependency chain and isn't GPU-parallelizable
// without a much larger reformulation (level scheduling or a switch to
// ILU0); Jacobi (Y[i] = X[i] / diag[i]) is embarrassingly parallel and
// lets the whole PCG loop -- matvec, dot products, vector updates, and
// the preconditioner -- run on the GPU without any per-iteration
// host<->device transfer. See test/electrostatic_gpu_solver_test.py for
// the correctness/benchmark check.
