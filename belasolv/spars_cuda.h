...
// the belasolv CMake target is built with -DENABLE_CUDA_SOLVER=ON; the
// declaration below is always visible so spars.cpp can call it behind a
// FEMM_CUDA_ENABLED compile-time guard. Electrostatics' linear system is
// always real-valued (no AC/harmonic formulation), so unlike fkn there is
// no complex-symmetric counterpart to port.
#pragma once
