...
void CbeladrawDoc::initalise_lua()
{
  ...
  lua_register(lua, "ei_savepng", lua_savepng);
  lua_register(lua, "ei_save_png", lua_savepng);
...
// ei_savepng"filename") -- the ei twin of mi_savepng.
...
int CbeladrawDoc::lua_savepng(lua_State* L)
{
  CatchNullDocument();
  CbeladrawDoc* thisDoc = (CbeladrawDoc*)pBeladrawDoc;
  POSITION pos = thisDoc->GetFirstViewPosition();
  CbeladrawView* theView = (CbeladrawView*)thisDoc->GetNextView(pos);
  ...
  if (GetScriptGui() == ScriptGui::Qt) {
    ...
    // (issue #87). Refused outright until #80 gave femmqt a reader for
    // this document type and #85 stopped an unrecognised file hanging
    // the render. A pre-processor view is geometry, so there is no
    // plot state to carry -- same as mi_savepng.
    CString pn = thisDoc->GetPathName();
    if (pn.GetLength() == 0) {
      CString msg = "ei_savepng: with setgui(\"qt\"), the document must "
    ...
    if (!RenderPngViaQtGui(theView->BinDir, pn, filename, width, height, NULL,
            &err)) {
      CString msg;
      msg.Format("ei_savepng: %s", (const char*)err);
      lua_error(L, msg.GetBuffer(1));
    }
    return 0;
  }
  ...
  if (!ok) {
    CString msg;
    msg.Format("ei_savepng: couldn't write \"%s\"", (const char*)filename);
    lua_error(L, msg.GetBuffer(1));
  }
  return 0;
}
