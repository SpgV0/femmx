...
void CbeladrawView::ApplyTheme(BOOL bDark)
{
  m_bDarkTheme = bDark;
  if (bDark) {
...
void CbeladrawView::OnViewDarkTheme()
{
  ApplyTheme(!DarkMode::IsEnabled());
}

void CbeladrawView::OnUpdateViewDarkTheme(CCmdUI* pCmdUI)
{
...
void CbeladrawView::OnRButtonDblClk(UINT nFlags, CPoint point)
{
  ...
  if (EditAction == 0) {
    ...
    if (i >= 0) {
      ...
      for (j = 0; j < pDoc->circproplist.GetSize(); j++) {
        if (pDoc->circproplist[j].CircName == pDoc->nodelist[i].InConductor) {
  ...
  if (EditAction == 1) {
    ...
    if (i >= 0) {
      ...
      sprintf(s, "<None>\n");
      for (j = 0; j < pDoc->circproplist.GetSize(); j++) {
        if (pDoc->circproplist[j].CircName == pDoc->linelist[i].InConductor) {
  ...
  if (EditAction == 3) {
    ...
    if (i >= 0) {
      ...
      ss += s;
      sprintf(s, "In Group:  %i\n", pDoc->arclist[i].InGroup);
      ss += s;

      ss += "In Conductor:  ";
      sprintf(s, "<None>\n");
      for (j = 0; j < pDoc->circproplist.GetSize(); j++) {
        if (pDoc->circproplist[j].CircName == pDoc->arclist[i].InConductor) {
  ...
  if (TheDoc->ProblemType == 1) {
    ...
    for (k = 0; k < TheDoc->blocklist.GetSize(); k++) {
      if (TheDoc->blocklist[k].IsExternal) {
        if ((TheDoc->extRi == 0) || (TheDoc->extRo == 0))
        ...
        for (i = 0; i < TheDoc->blockproplist.GetSize(); i++)
          if (TheDoc->blocklist[k].BlockType == TheDoc->blockproplist[i].BlockName) {
            if (TheDoc->blockproplist[i].ex != TheDoc->blockproplist[i].ey)
              bFlag = TRUE;
          }
      }
    }
...
void CbeladrawView::OnSwitchToQtGui()
{
  CbeladrawDoc* TheDoc = GetDocument();
  ASSERT_VALID(TheDoc);
