...
void CbelaviewView::ApplyTheme(BOOL bDark)
{
  m_bDarkTheme = bDark;
  if (bDark) {
...
void CbelaviewView::OnViewDarkTheme()
{
  ApplyTheme(!DarkMode::IsEnabled());
}

void CbelaviewView::OnUpdateViewDarkTheme(CCmdUI* pCmdUI)
{
...
void CbelaviewView::PlotFluxDensity(CDC* pDC, int elmnum, int flag)
{
  ...
  for (i = 0; i < 3; i++) {
    ...
    case 3:
      if (pDoc->Smooth == TRUE)
        bn[i] = abs(pDoc->e(elmnum, i));
      else
        bn[i] = abs(pDoc->E(elmnum));
      break;
...
BOOL CbelaviewView::Pump()
{
  ...
  // of *this* OnDraw call for the OLD ox/oy/mag, and density-plot
  // drawing additionally clears/refills the shared m_densityBandPts
  // buffers -- exactly the "now-stale cached coordinates" hazard ff79e58
  // set out to fix, just still present for PM_QS_INPUT instead of
  // PM_QS_PAINT. Only sent-message traffic (cross-thread ::SendMessage,
  // e.g. from COM automation) needs servicing here so the app doesn't
  // look hung to whoever's waiting on one; peeking (not dispatching)
  // input is enough for bCancel above to unwind the caller's loop, and
  // leaving that input queued means the normal message loop dispatches
  // it non-reentrantly once OnDraw actually returns, whose own
...
void CbelaviewView::OnDraw(CDC* pDC)
{
  ...
  if (MeshFlag == TRUE && !bCanceled) {
    ...
    // single PolyPolyline() call every kMaxMeshEdgePts points, instead of
    // one MoveTo()+LineTo() GDI call pair per edge -- for a large mesh
    // (tens/hundreds of thousands of elements) that's hundreds of
    // thousands of individual GDI calls. MyLineTo() also calls
    // GetClientRect() on every single edge just for its off-screen
    // clamping, a Win32 round trip; ClipMeshSegment() (above) does the
    // same clamping math but takes the client-rect-derived bound as a
    // parameter, fetched once here instead. Capped like the density-plot
    // bands: PolyPolyline() with an unbounded point count is the same
    // known GDI perf/crash cliff.
    RECT crMesh;
      ...
      if ((pDoc->meshelem[i].lbl == DrawSelected) || (DrawSelected == -1)) {
        ...
        for (j = 0; j < 3; j++) {
          ...
          if ((po > pi) || (OnBoundary == TRUE)) {
            POINT clipped[2];
            if (ClipMeshSegment(pDoc->meshnode[pi].xs, pDoc->meshnode[pi].ys,
                    pDoc->meshnode[po].xs, pDoc->meshnode[po].ys, ZmMesh, clipped)) {
          ...
          for (i = ilo; i <= ihi && !bCanceled; i++) {
            for (j = jlo; j <= jhi && !bCanceled; j++) {
              ...
              if (pDoc->InTriangleTest(xd, yd, k)) {
                ...
                if (VectorPlot == 1)
                  vr = (u.D) * GridSize * VectorScaleFactor / MaxVal;
                else
                  vr = (u.E) * GridSize * VectorScaleFactor / MaxVal;
                DwgToScreen(xd, yd, &xs, &ys, &r);
                MyMoveTo(pDC, xs, ys);
                xp = xs;
                yp = ys;
                ...
                MyLineTo(pDC, xs, ys);
                if ((xs == xp) && (ys == yp))
                  pDC->SetPixel((int)xs, (int)ys, VectorColor);
                else if (abs(vr) != 0) {
                  xp = xs;
                  yp = ys;
  ...
  if ((LegendFlag == TRUE) && (DensityPlot != 0) && !bCanceled) {
    ...
    if (DensityPlot == 2)
      sprintf(cc, "Density Plot: |D|, C/m^2");
    if (DensityPlot == 3)
    ...
    if (dlg.DoModal() == IDOK) {
      ...
      case 3:
        ...
        if (pDoc->ProblemType == 0)
          ...
          sprintf(s, "Force in z-direction = %g N", z[1]);
        MyMessageBox(s);
        break;
...
void CbelaviewView::OnSwitchToQtGui()
{
  CbelaviewDoc* TheDoc = GetDocument();
  ASSERT_VALID(TheDoc);
