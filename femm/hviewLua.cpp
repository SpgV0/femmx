...
int ChviewDoc::lua_savepng(lua_State* L)
{
  ...
  if (GetScriptGui() == ScriptGui::Qt) {
    ...
    if (pn.GetLength() == 0) {
      CString msg = "ho_savepng: with setgui(\"qt\"), the solution must "
    ...
    if (!QtDensityPlotIsRenderable(QtRenderPhysics::HeatFlow,
            theView->DensityPlot, &wanted, &renderable)) {
      CString msg;
      msg.Format("ho_savepng: the Qt renderer draws one density quantity for this "
    ...
    if (!RenderPngViaQtGui(theView->BinDir, pn, filename, width, height, &plot,
            &err)) {
      CString msg;
      msg.Format("ho_savepng: %s", (const char*)err);
      lua_error(L, msg.GetBuffer(1));
    }
    return 0;
  }
