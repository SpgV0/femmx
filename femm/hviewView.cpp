...
void ChviewView::OnDraw(CDC* pDC)
{
          ...
          for (i = ilo; i <= ihi; i++) {
            for (j = jlo; j <= jhi; j++) {
              ...
              if (pDoc->InTriangleTest(xd, yd, k)) {
                ...
                if (VectorPlot == 1)
                  vr = (u.F) * GridSize * VectorScaleFactor / MaxVal;
                else
                  vr = (u.G) * GridSize * VectorScaleFactor / MaxVal;
                DwgToScreen(xd, yd, &xs, &ys, &r);
                MyMoveTo(pDC, xs, ys);
                xp = xs;
                yp = ys;
  ...
  if ((LegendFlag == TRUE) && (DensityPlot != 0) && !bCanceled) {
    ...
    if (DensityPlot == 1)
      sprintf(cc, "Density Plot: Temperature (K)");
    if (DensityPlot == 2)
      sprintf(cc, "Density Plot: |F|, W/m^2");
    if (DensityPlot == 3)
      sprintf(cc, "Density Plot: |G|, K/m");
...
void ChviewView::OnSwitchToQtGui()
{
  ChviewDoc* TheDoc = GetDocument();
  ASSERT_VALID(TheDoc);
