    ...
    : QMainWindow(parent)
{
  ...
  // Distinct, always-visible button for jumping back to the classic MFC
  // GUI, per direct user request -- the toolbars above are deliberately
  // icon-only (see their own comment), so "Switch to Classic GUI" living
  // only as a File-menu item was easy to miss. Pinned to the menu bar's
  // corner so it stays visible regardless of window size, not swallowed
  // into an overflow chevron the way a toolbar button could be.
  auto* switchToClassicButton = new QPushButton("Switch to Classic GUI", this);
  switchToClassicButton->setToolTip("Close this file here and reopen it in the original FEMM interface");
  connect(switchToClassicButton, &QPushButton::clicked, this, &MainWindow::onSwitchToClassicTriggered);
  menuBar()->setCornerWidget(switchToClassicButton, Qt::TopRightCorner);
