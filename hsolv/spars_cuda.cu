...
// new file. Optional CUDA-accelerated linear solve for the heat-flow
// solver, ported from fkn/spars_cuda.cu's CudaPCGSolve (the real-valued
// case only -- heat flow has no complex/AC formulation, so
...
// host<->device transfer. See test/thermal_gpu_solver_test.py for the
// correctness/benchmark check.
