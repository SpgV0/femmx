...
% documented ci_setgpuaccel (Problem Commands), added alongside the
% CUDA-accelerated linear solve ported to csolv (see csolv/spars_cuda.cu).
% Also documented ci_setredraw (Editing Commands), mirroring magnetics'
% mi_setredraw (see femm/CDRAWLUA.CPP).
...
\item{\tt ci\_setgpuaccel(flag)} Sets whether {\tt csolv} should try
...
for the normal CPU solve. If {\tt csolv} was not built with CUDA
...
\item{\tt ci\_setredraw(flag)} Suspends or resumes the canvas redraw
while editing. Set {\tt flag} to {\tt 0} to suspend redraw before a
batch of edit operations ({\em e.g.} many {\tt ci\_copytranslate} or
{\tt ci\_copyrotate} calls on a densely-drawn model), and back to {\tt
...
\item \texttt{ci\_getmaterial("materialname", "foldername")} fetches the
