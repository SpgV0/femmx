...
% documented ei_setgpuaccel (Problem Commands), added alongside the
% CUDA-accelerated linear solve ported to belasolv (see
% belasolv/spars_cuda.cu). Also documented ei_setredraw (Editing
% Commands), mirroring magnetics' mi_setredraw (see femm/beladrawLua.cpp).
...
\item{\tt ei\_setgpuaccel(flag)} Sets whether {\tt belasolv} should try
...
for the normal CPU solve. If {\tt belasolv} was not built with CUDA
...
the problem file, mirroring magnetics' {\tt mi\_setgpuaccel}.
...
\item{\tt ei\_setredraw(flag)} Suspends or resumes the canvas redraw
while editing. Set {\tt flag} to {\tt 0} to suspend redraw before a
batch of edit operations ({\em e.g.} many {\tt ei\_copytranslate} or
{\tt ei\_copyrotate} calls on a densely-drawn model), and back to {\tt
1} (the default state) afterward to resume it and force a single
refresh. Mirrors magnetics' {\tt mi\_setredraw}.
...
\item \texttt{ei\_getmaterial("materialname", "foldername")} fetches the
