...
% documented hi_setgpuaccel (Problem Commands), added alongside the
% CUDA-accelerated linear solve ported to hsolv (see hsolv/spars_cuda.cu).
% Also documented hi_setredraw (Editing Commands), mirroring magnetics'
% mi_setredraw (see femm/HDRAWLUA.CPP).
...
\item{\tt hi\_setgpuaccel(flag)} Sets whether {\tt hsolv} should try
...
for the normal CPU solve. If {\tt hsolv} was not built with CUDA
...
\item{\tt hi\_setredraw(flag)} Suspends or resumes the canvas redraw
while editing. Set {\tt flag} to {\tt 0} to suspend redraw before a
batch of edit operations ({\em e.g.} many {\tt hi\_copytranslate} or
{\tt hi\_copyrotate} calls on a densely-drawn model), and back to {\tt
...
\item \texttt{hi\_getmaterial("materialname", "foldername")} fetches the
