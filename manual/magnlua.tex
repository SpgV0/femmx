...
% documented mi_setgpuaccel (Problem Commands) and mi_setredraw (Editing
% Commands), the two Lua commands added since the femm-4.2-22Oct2023
% pre-fork baseline (see femm/femmeLua.cpp).
% Modified by Claude (Anthropic), noreply@anthropic.com, 2026-07-09:
% documented get_solve_stats() (Common Lua Command Set) -- reports the
% last solve's CPU/GPU/RAM/duration stats from the Load window, for
% benchmarking scripts (see femm/femm.cpp's lua_getsolvestats).
...
\item {\tt get\_solve\_stats()} Returns CPU/GPU/RAM usage and execution
time for the most recently completed solve (magnetics, heat flow,
electrostatics, or current flow), as tracked by the CPU/GPU/RAM Load
window (View menu). Intended for benchmarking scripts. Returns seven
values, in order: {\tt exec\_time} (seconds), {\tt cpu\_max}, {\tt
cpu\_avg}, {\tt gpu\_max}, {\tt gpu\_avg}, {\tt ram\_max}, {\tt
ram\_avg}. All usage figures are percentages from 0 to 100. {\tt
gpu\_max} and {\tt gpu\_avg} are {\tt -1} if no NVIDIA GPU was
available to sample. Raises a Lua error if the Load window is
disabled, or if no {\tt mi\_/hi\_/ei\_/ci\_analyze} has completed yet
in this session. Example:
\begin{verbatim}
mi_analyze(1)
exec_time,cpu_max,cpu_avg,gpu_max,gpu_avg,ram_max,ram_avg =
    get_solve_stats()
\end{verbatim}
...
\item {\tt setgui("name")} Chooses which GUI renders for the script
commands that produce GUI-derived output --- at present {\tt mi\_savepng}
and {\tt mo\_savepng}. Accepts {\tt "classic"} (equivalently {\tt "old"},
{\tt "femm"}, {\tt "mfc"} or {\tt 0}) for the traditional MFC interface,
and {\tt "qt"} ({\tt "new"}, {\tt "femmqt"} or {\tt 1}) for the newer Qt
interface. Lua itself always runs inside the classic application, so this
does not move the script; under {\tt "qt"} the commands above hand the
saved file to {\tt femmqt.exe}, which draws it with its own antialiased
density/contour plotting.

The setting lasts for the current session only. It starts out matching the
{\tt <PreferredGUI>} entry in {\tt femm.cfg}, so a script that never calls
{\tt setgui} renders with whichever GUI is preferred, but {\tt setgui}
deliberately does not write that entry back --- a script asking for one
image in a particular style should not change which GUI opens by default
afterwards. Use the {\tt View} menu's {\tt Switch To\ldots} item for that.

\item {\tt getgui()} Returns the current setting as a string, either
{\tt "classic"} or {\tt "qt"}.
...
\section{Commands Added Since FEMM 4.2}

Every command below is specific to FEMMX and is not present in FEMM
4.2. They are documented in full alongside their peers in the sections
that follow; this list exists so that a script written for FEMM 4.2 can
be told apart from one that requires FEMMX.

\begin{itemize}
\item {\tt get\_solve\_stats()} --- returns the last solve's duration
and CPU/GPU/RAM usage, for benchmarking scripts.
\item {\tt setgui("name")} / {\tt getgui()} --- choose which GUI renders
for the commands that produce GUI-derived output.
\item {\tt mi\_setgpuaccel(flag)}, {\tt ei\_setgpuaccel(flag)},
{\tt hi\_setgpuaccel(flag)} and {\tt ci\_setgpuaccel(flag)} --- opt the
corresponding solver into CUDA-accelerated solving. All four solvers
carry the CUDA port and all four commands are registered; earlier
revisions of this list credited only the first two.
\item {\tt mi\_setredraw(flag)}, {\tt ei\_setredraw(flag)},
{\tt hi\_setredraw(flag)} and {\tt ci\_setredraw(flag)} --- suppress
redraws during a scripted sequence of edits. As with every other
command, the underscore-separated spelling ({\tt mi\_set\_redraw}, and
so on) works too.
\item {\tt mi\_savepng("filename")}, {\tt ei\_savepng("filename")},
{\tt hi\_savepng("filename")} and {\tt ci\_savepng("filename")} in the
editors, and {\tt mo\_savepng}, {\tt eo\_savepng}, {\tt ho\_savepng} and
{\tt co\_savepng} in the post-processors --- save a PNG screenshot; FEMM
4.2 could only write bitmaps and metafiles. The magnetics pair came
first; the other six were added later, since a script working in any
other physics could otherwise produce nothing but BMP or WMF.

All eight now honour {\tt setgui("qt")}. The six non-magnetics commands
refused it until {\tt femmqt} gained an editor and a Solution Viewer for
all four physics; what remains of that refusal is narrower and more
useful. The Qt Solution Viewer draws exactly one density quantity for
electrostatics, heat flow and current flow --- the field its solver
stores, namely {\tt |D|} in C/m$^2$, {\tt |F|} in W/m$^2$ and {\tt |J|}
in A/m$^2$ --- so if the view is showing the potential or the other
gradient instead, {\tt eo\_savepng} and its two siblings say so by name
and ask for {\tt setgui("classic")}, rather than rendering the one
quantity they do have and letting it pass for the one requested.
Contour plots, which show the potential, are drawn for every physics.
\end{itemize}
...
\item{\tt mi\_setgpuaccel(flag)} Sets whether {\tt fkern} should try
its optional CUDA-accelerated linear solve for this problem. Set {\tt
flag} to {\tt 1} to request GPU acceleration, or {\tt 0} (the default)
for the normal CPU solve. If {\tt fkern} was not built with CUDA
support, or if no usable GPU is found at run time, the solve
transparently falls back to the CPU solver. This setting is saved with
the problem file (like the AC solver choice), and can also be set via
the ``Use GPU Acceleration'' checkbox in the Problem Definition
dialog.
...
\item{\tt mi\_setredraw(flag)} Suspends or resumes the canvas redraw
while editing. Set {\tt flag} to {\tt 0} to suspend redraw before a
batch of edit operations ({\em e.g.} many {\tt mi\_copytranslate} or
{\tt mi\_copyrotate} calls on a densely-drawn model), and back to {\tt
1} (the default state) afterward to resume it and force a single
refresh showing everything that changed while it was off. Useful for
speeding up scripts that perform many repeated copy or move operations
on a model that already has a lot of geometry drawn.
\end{itemize}
...
\item \texttt{mi\_getmaterial("materialname", "foldername")} fetches the
material of that name from the library folder named \texttt{foldername}.
The shipped libraries contain entries in different folders that share a
name with a materially different material -- \texttt{Supermalloy} appears
in both Nickel Alloys and Metals Handbook DC Magnetization Curves with
relative permeabilities differing by a factor of half a million, and
\texttt{Ammonia} appears in heatlib.dat as both a saturated liquid and a
gas. Without a folder the first entry in file order wins and the other
cannot be reached from a script at all. Any level of the folder path may
be given. Asking for a folder that holds no such material is an error
rather than a fallback to the name-only match.
...
\item{\tt mi\_savepng("filename")} saves a PNG screenshot of the current
view to the file specified by {\tt "filename"}, following the same file
name rules as {\tt mi\_savebitmap} above. Which GUI does the drawing
depends on the current {\tt setgui} setting: under {\tt setgui("classic")}
the traditional view renders itself and the result is encoded as PNG,
while under {\tt setgui("qt")} the file --- which must already be saved to
disk --- is handed to {\tt femmqt.exe}, which renders it offscreen with its
own antialiased plotting. An optional pixel size may be given as
{\tt mi\_savepng("filename",w,h)}; it applies only to the Qt renderer,
since the classic one can only capture its window at the size that window
actually is.
...
\item{\tt mo\_savepng("filename")} saves a PNG screen shot of the current
view to the file specified by {\tt "filename"}, following the same file
name rules as {\tt mo\_savebitmap} above. Which GUI does the drawing
depends on the current {\tt setgui} setting: under {\tt setgui("classic")}
the traditional post-processor view renders itself and the result is
encoded as PNG, while under {\tt setgui("qt")} the solution --- which must
already be saved to disk --- is handed to {\tt femmqt.exe}, which renders
it offscreen with its own antialiased density and contour plotting. An
optional pixel size may be given as {\tt mo\_savepng("filename",w,h)}; it
applies only to the Qt renderer, since the classic one can only capture
its window at the size that window actually is.

Under {\tt setgui("qt")} the post-processor's current view settings are
carried across to the Qt renderer, so that {\tt mo\_showdensityplot} and
friends affect the saved image as they do the on-screen one. What
crosses:

\begin{itemize}
\item the plot mode --- density or contour;
\item the density quantity ({\tt "bmag"}, {\tt "hreal"}, {\tt "logb"} and
      the rest), correctly interpreted for DC or AC, since the same
      index means different quantities in the two cases;
\item the density plot's lower and upper bounds;
\item the greyscale flag and the legend flag;
\item the number of contour lines;
\item the visible region. This matters for more than framing: the
      density plot's colour banding is scaled to what is {\em visible},
      so a full-model render cannot reproduce a zoomed-in view's
      colours.
\end{itemize}

What does {\em not} cross, and why:

\begin{itemize}
\item {\tt mo\_showvectorplot} --- the Qt viewer has no vector overlay
      at all, so a vector plot has no counterpart to render;
\item the contour mask and the real/imaginary contour selection
      ({\tt mo\_showcontourplot}'s {\tt "real"}/{\tt "imag"}/{\tt "both"}
      argument) --- the contour {\em count} and bounds do cross, the
      selection does not;
\item smoothing, mesh display and point display, which are the Qt
      viewer's own settings rather than part of what a saved image was
      asked for.
\end{itemize}

If you need any of those in a saved image, use {\tt setgui("classic")},
which captures exactly what the traditional post-processor is showing.
