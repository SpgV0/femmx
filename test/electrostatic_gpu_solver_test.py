"""
Correctness + benchmark test for belasolv.exe's optional CUDA-accelerated
linear solve (see belasolv/spars_cuda.cu, femm/beladrawLua.cpp's
ei_setgpuaccel), mirroring test/gpu_solver_test.py's magnetics test for
the electrostatics solver.

Builds one moderately large electrostatic problem (an annulus with a
fixed-voltage inner boundary and fixed-voltage outer boundary, fine-meshed
enough to get tens of thousands of nodes -- GPU acceleration is not
expected to help on FEMM's typical small problems; see
...
with ei_setgpuaccel(0) (CPU, the default) and once with ei_setgpuaccel(1)
(GPU if available), and checks:

  1. Correctness: both solves agree on the computed voltage at a probe
     point to within a tight tolerance. This holds whether or not this
     build of belasolv.exe actually has CUDA support -- if it doesn't,
     ei_setgpuaccel(1) is a no-op and both runs are the same CPU solve,
  ...
  2. Speedup: only asserted if this build appears to actually have CUDA
     ...
     belasolv.exe, per belasolv/CMakeLists.txt's ENABLE_CUDA_SOLVER
     install step) -- otherwise this is skipped, since there's nothing
     to speed up.

Requirements: pip install pyfemm pywin32; a built + COM-registered
femmx.exe. For the speedup assertion specifically: belasolv.exe built
with -DENABLE_CUDA_SOLVER=ON (see belasolv/CMakeLists.txt) and a
CUDA-capable GPU.

Usage:
    pytest electrostatic_gpu_solver_test.py -v
    python electrostatic_gpu_solver_test.py
"""
...
def build_model(model_path, gpu):
    femm.newdocument(1)
    femm.ei_probdef("millimeters", "planar", 1e-8, 1, 30)
    femm.ei_addmaterial("Dielectric", PERMITTIVITY, PERMITTIVITY, 0)
    femm.ei_addboundprop("Hot", V_HOT, 0, 0, 0, 0)
    femm.ei_addboundprop("Cold", V_COLD, 0, 0, 0, 0)

    femm.ei_addnode(INNER_RADIUS_MM, 0)
    femm.ei_addnode(-INNER_RADIUS_MM, 0)
    femm.ei_addarc(INNER_RADIUS_MM, 0, -INNER_RADIUS_MM, 0, 180, 2.5)
    femm.ei_addarc(-INNER_RADIUS_MM, 0, INNER_RADIUS_MM, 0, 180, 2.5)
    femm.ei_selectarcsegment(0, INNER_RADIUS_MM)
    femm.ei_setarcsegmentprop(2.5, "Hot", 0, 0, "")
    femm.ei_clearselected()
    femm.ei_selectarcsegment(0, -INNER_RADIUS_MM)
    femm.ei_setarcsegmentprop(2.5, "Hot", 0, 0, "")
    femm.ei_clearselected()

    femm.ei_addnode(OUTER_RADIUS_MM, 0)
    femm.ei_addnode(-OUTER_RADIUS_MM, 0)
    femm.ei_addarc(OUTER_RADIUS_MM, 0, -OUTER_RADIUS_MM, 0, 180, 2.5)
    femm.ei_addarc(-OUTER_RADIUS_MM, 0, OUTER_RADIUS_MM, 0, 180, 2.5)
    femm.ei_selectarcsegment(0, OUTER_RADIUS_MM)
    femm.ei_setarcsegmentprop(2.5, "Cold", 0, 0, "")
    femm.ei_clearselected()
    femm.ei_selectarcsegment(0, -OUTER_RADIUS_MM)
    femm.ei_setarcsegmentprop(2.5, "Cold", 0, 0, "")
    femm.ei_clearselected()
    ...
    femm.ei_addblocklabel(OUTER_RADIUS_MM / 2, 0)
    femm.ei_selectlabel(OUTER_RADIUS_MM / 2, 0)
    femm.ei_setblockprop("Dielectric", 0, MESH_SIZE_MM, 0)
    femm.ei_clearselected()

    if gpu:
        femm.callfemm("ei_setgpuaccel(1)")

    femm.ei_saveas(model_path)
    femm.ei_createmesh()
...
def solve_and_measure(model_path, gpu):
    ...
    try:
        ...
        values = femm.eo_getpointvalues(*PROBE_POINT)
        voltage = values[0]
    finally:
        try:
            femm.eo_close()
...
def gpu_vs_cpu_result():
    ...
    cpu_path = os.path.join(RESULTS_DIR, "electrostatic_gpu_solver_cpu.fee")
    gpu_path = os.path.join(RESULTS_DIR, "electrostatic_gpu_solver_gpu.fee")

    v_cpu_val, t_cpu = solve_and_measure(cpu_path, gpu=False)
    v_gpu_val, t_gpu = solve_and_measure(gpu_path, gpu=True)

    rel_diff_pct = abs(v_gpu_val - v_cpu_val) / abs(v_cpu_val) * 100
    speedup = t_cpu / t_gpu if t_gpu > 0 else float("inf")
    cuda_available = _cuda_build_available()

    lines = [
        "GPU-accelerated electrostatic solve: correctness + benchmark",
        "==============================================================",
        f"CUDA-enabled build detected: {cuda_available}",
        f"Probe point: {PROBE_POINT}",
        "",
        f"CPU: V = {v_cpu_val:.6e}  (time: {t_cpu:.3f}s)",
        f"GPU: V = {v_gpu_val:.6e}  (time: {t_gpu:.3f}s)",
...
def test_gpu_is_faster_when_available(gpu_vs_cpu_result):
    if not gpu_vs_cpu_result["cuda_available"]:
        pytest.skip(
            "no CUDA runtime DLLs found next to belasolv.exe -- this build "
